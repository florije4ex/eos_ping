...
function cleos() {
  docker exec keosd cleos -u http://nodeosd:8888 --wallet-url http://localhost:8900 "$@"
}

function eosiocpp() {
  docker exec keosd eosiocpp "$@"
}

function keosd() {
  docker exec nodeosd keosd "$@"
}

function pkill() {
  docker exec nodeosd pkill "$@"
}
